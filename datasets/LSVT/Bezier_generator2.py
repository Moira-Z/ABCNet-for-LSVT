...
for il, label in labels.items():
    print('Processing: '+ il.split('_')[-1])
    imgdir = r'./train_full_images/' + il.split('_')[-1] + '.jpg'
    if os.path.exists('./abcnet_gen_labels/' + il + '.txt') and il.split('_')[-1] != 20733:
        print('done:' + il)
        continue 
    
    outgt = open('./abcnet_gen_labels/' + il + '.txt', 'w', encoding="utf-8")
    ...
    for box in label:
        ...
        if ct == '###': continue
        coords = [(float(ix[0]), float(ix[1])) for ix in pts]
        poly = Polygon(coords)

        data.append(np.array([float(i) for x in pts for i in x]))
        cts.append(ct)
        polys.append(poly)
